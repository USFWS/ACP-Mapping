...
birdsQC <- birds %>% cbind(st_coordinates(.)) %>%
  ...
  #
  # design <- st_read(dsn=paste("Design_Transects/ACP_",Y,"_Transects.shp", sep="")) %>%
  # st_transform(crs=3338)
  # birdsQC <- birds %>% cbind(st_coordinates(.)) %>% filter(Year == 2013, Transect %in%c(38, 45))
  # df <- filter(design, TRANSID %in% c(38, 45)) %>% st_distance(birdsQC) %>% t() %>%
  # as.data.frame() %>% rename(to_38 = V1, to_45=V2) %>%
  # bind_cols(birdsQC)
  # view(birdQC)
  # HMW seems to have reversed transect 38 and 45 on day 10
  ## Solution: reverse transect 38 and 45 on day 10
  mutate(Transect = if_else(Year == 2013 & Observer == "HMW" & Day == 10, 
                            if_else(Transect == 38, 45, 38), 
                            Transect))
#Did it work?
...
tm <- tm_shape(acp) + tm_polygons(col = "STRATNAME", alpha = 0.5) + 
  tm_shape(df, name=paste(Y, "Flown Track")) + tm_lines() + 
  tm_text("Transect", size=2) + 
  tm_shape(bdf, name=paste(Y,"Bird Obs")) + tm_dots(col="Observer") + 
  tm_basemap(server = "Esri.WorldGrayCanvas") + 
  tm_scale_bar() 
tm
